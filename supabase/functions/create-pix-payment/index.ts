...
serve(async (req) => {
  ...
  try {
    ...
    if (amount < 5) {
      return new Response(
        JSON.stringify({ error: 'Minimum amount is R$ 5.00' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    const paymentData = {
      ...
      payer: {
        ...
        identification: {
          type: 'CPF',
          number: '11122233344' // In production, get from user profile
        }
      },
      notification_url: `${Deno.env.get('SUPABASE_URL')}/functions/v1/mercado-pago-webhook`,
      external_reference: user.id, // To identify the user when webhook arrives
    };
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to create payment', details: responseData }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
