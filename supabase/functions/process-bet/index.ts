...
serve(async (req) => {
  ...
  try {
    ...
    const validNumbers = chosenNumbers.every(num => 
      Number.isInteger(num) && num >= 0 && num <= 99
    );
    ...
    if (contest.status !== 'open') {
      return new Response(
        JSON.stringify({ error: 'Contest is not open for bets' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (contest.closing_date && new Date() > new Date(contest.closing_date)) {
      return new Response(
        JSON.stringify({ error: 'Contest betting period has ended' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    const betAmount = 5.00; // Fixed bet amount
    ...
    if (!wallet || wallet.balance < betAmount) {
      return new Response(
        JSON.stringify({ error: 'Insufficient balance. Please deposit funds to your wallet.' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (updateWalletError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to process bet. Please try again.' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    const { data: bet, error: betError } = await supabase
      ...
      .insert({
        ...
        chosen_numbers: chosenNumbers.sort((a, b) => a - b), // Sort numbers for consistency
        amount: betAmount,
        payment_status: 'completed', // Since we debited from wallet
      })
      .select()
      .single();
    ...
    if (betError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to create bet. Wallet balance restored.' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
