...
serve(async (req)=>{
  ...
  try {
    ...
    const winningNumbers = body?.winningNumbers;
    if (!contestId || !Array.isArray(winningNumbers)) {
      return new Response(JSON.stringify({
        error: "contestId and winningNumbers are required"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    if (!valid) {
      return new Response(JSON.stringify({
        error: "Winning numbers must be 20 unique integers between 0 and 99"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    // Load contest
    const { data: contest, error: contestErr } = await supabaseAdmin.from("contests").select("id, status, total_collected, month_year").eq("id", contestId).single();
    ...
    const { error: updContestErr } = await supabaseAdmin.from("contests").update({
      status: "closed",
    ...
    const { data: bets, error: betsErr } = await supabaseAdmin.from("bets").select("id, user_id, chosen_numbers").eq("contest_id", contest.id);
    if (betsErr) {
    ...
    for (const bet of bets ?? []){
      const chosen = bet.chosen_numbers ?? [];
      const hits = chosen.reduce((acc, n)=>winSet.has(Number(n)) ? acc + 1 : acc, 0);
      const { error: updBetErr } = await supabaseAdmin.from("bets").update({
        hits
      }).eq("id", bet.id);
      if (updBetErr) console.error("Update hits error for bet", bet.id, updBetErr);
      if (hits === 6) winners6.push({
        bet_id: bet.id,
        user_id: bet.user_id,
        hits
      });
      else if (hits === 5) winners5.push({
        bet_id: bet.id,
        user_id: bet.user_id,
        hits
      });
    ...
    //busca os rateios 
    const { data: rateio, error: rateioErr } = await supabaseAdmin.from("rateios").select("house_share, six_hits_share, five_hits_share").single();
    if (rateioErr || !rateio) {
      console.error("Erro ao buscar rateios:", rateioErr);
      return new Response(JSON.stringify({
        error: "Erro ao buscar configurações de rateio"
      }), {
        status: 500
      });
    }
    // Definição das variáveis de configuração
    const house_share = Number(rateio.house_share)/100;
    const winner6_share = Number(rateio.six_hits_share)/100;
    const winner5_share = Number(rateio.five_hits_share)/100;
    ...
    const pool6 = count6 > 0 ? Math.round(prizeValue * winner6_share * 100) / 100 : 0;
    const pool5 = count5 > 0 ? Math.round(prizeValue * winner5_share * 100) / 100 : 0;
    // Round to cents
    const round2 = (v)=>Math.round(v * 100) / 100;
    // Credit prizes and mark bets
    if (count6 > 0) {
      const each = round2(pool6 / count6 * (1 - house_share)); // deduct 20% admin fee per prize
      for (const w of winners6){
        // Update wallet (fetch, add, update)
        const { data: wallet, error: wErr } = await supabaseAdmin.from("wallets").select("id, balance").eq("user_id", w.user_id).single();
        if (wErr) {
          console.error("Wallet fetch error for", w.user_id, wErr);
        } else {
          const newBal = round2(Number(wallet?.balance || 0) + each);
          const { error: upWErr } = await supabaseAdmin.from("wallets").update({
            balance: newBal
          }).eq("id", wallet.id);
          if (upWErr) console.error("Wallet update error", upWErr);
        }
        // Mark bet prize
        const { error: updBetPrizeErr } = await supabaseAdmin.from("bets").update({
          prize_amount: each,
          prize_paid: true
        }).eq("id", w.bet_id);
    ...
    if (count5 > 0) {
      const each = round2(pool5 / count5 * (1 - house_share)); // deduct 20% admin fee per prize
